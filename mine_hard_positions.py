...
def main():
    ...
    with torch.inference_mode():
        for batch in DataLoader(dataset, batch_size=args.batch_size):
            target_p, target_v = batch["policy"].to(device), batch["wdl"].to(device); out = model(batch["position"].to(device)); logits = out["policy"].masked_fill(~batch["legal_mask"].to(device), torch.finfo(out["policy"].dtype).min); pred_p = torch.softmax(logits, 1); pred_v = torch.softmax(out["wdl"], 1)
    ...
    print(f"wrote {len(selected)} rankings to {args.output} and positions to {fen_output}")
