...
def neural_metrics(model, loader, device):
    ...
    for batch in loader:
        ...
        out = model(position); logp = F.log_softmax(out["policy"], 1); pred_v = torch.softmax(out["wdl"], 1)
        ...
        teacher_top = target_p.argmax(1); top3 = out["policy"].topk(3, 1).indices
        totals["top1"] += float((out["policy"].argmax(1) == teacher_top).sum().cpu())
